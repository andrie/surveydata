...
library(dplyr)
```
```{r motivation, output.lines = 9}
sv <- membersurvey %>% as.tbl()
sv
```

Notice from this summary that Question 1 has two columns, i.e. `Q1_1` and `Q1_2`.  You can extract both these columns by simply referring to `Q1`:
  
```{r motivation-q1}
sv[, "Q1"]
```

However, the subset of `Q2` returns only a single column:
...
Using `qText()` without the question argument returns the text of all questions:
